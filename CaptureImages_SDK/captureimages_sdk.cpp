#include "captureimages_sdk.h"

 CaptureImages_SDK* CaptureImages_SDK::pThis=nullptr;

CaptureImages_SDK::CaptureImages_SDK(QObject *parent)
{
    this->setParent(parent);

    CaptureImages_SDK::pThis=this;
...
CaptureImages_SDK::~CaptureImages_SDK()
{
    delete CaptureImages_SDK::pThis;
    CaptureImages_SDK::pThis=nullptr;
...
void CaptureImages_SDK::releaseResourcesSlot()
{
...
void CaptureImages_SDK::exceptionCallBack_V30(DWORD dwType, LONG lUserID, LONG lHandle, void *pUser)
{
...
void CaptureImages_SDK::loginResultCallBack(LONG lUserID, DWORD dwResult, LPNET_DVR_DEVICEINFO_V30 lpDeviceInfo, void *pUser)
{
    ...
    if(dwResult==0){
        ...
        if(pThis->NET_DVR_GetLastError_L()==7){/* 网络异常,超时 */
            pThis->initCamerSlot(pThis->camerIp,pThis->port,pThis->camerName,pThis->camerPow,CaptureImages_SDK::pThis->alias);/* 重新登录 */
        }
    }
    else if (dwResult==1) {
...
void CaptureImages_SDK::initCamerSlot(const QString &camerIP, const int &camerPort, const QString &CamerUser, const QString &CamerPow, const QString &alias)
{
    if(!isSDKInit){
        ...
        LoginInfo.cbLoginResult=CaptureImages_SDK::loginResultCallBack;
        ...
        if(NET_DVR_Init_L!=nullptr){
            if(NET_DVR_Init_L()){
                ...
                if(NET_DVR_SetExceptionCallBack_V30_L){
                    NET_DVR_SetExceptionCallBack_V30_L(0,nullptr,CaptureImages_SDK::exceptionCallBack_V30,nullptr);
...
bool CaptureImages_SDK::putCommandSlot(const int &imgNumber, const QString &imgTime)
{
    if(dwResult){
...
void CaptureImages_SDK::playStreamSlot(quint64 winID, bool play)
{
    if(dwResult){
        if(play){
...
void CaptureImages_SDK::resizeEventSlot()
{
    if(dwResult){
...
void CaptureImages_SDK::getDeviceStatusSlot()
{    
